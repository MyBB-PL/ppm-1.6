...
function upgrade17_dbchanges6()
{
	...
	$output->print_header("Naprawa adresów IP postów po konwersji");
	...
	echo "<p>Naprawianie adresu z {$lower} na {$upper} (Łącznie: {$cnt['ipcount']})</p>";
	flush();
	...
	else
	{
		...
		$contents = "<p>Zakończono</p><p>Wszystkie adresy IP zostały naprawione. Naciśnij przycisk Dalej, aby kontynuować.</p>";
	}
	$output->print_contents($contents);
...
function upgrade17_dbchanges7()
{
	...
	$output->print_header("Naprawa adresów IP użytkowników po konwersji");
	...
	$contents .= "<p>Naprawianie adresu IP z {$lower} do {$upper} (Łącznie: {$cnt['ipcount']})</p>";
	...
	if($remaining && $ipaddress)
	{
		$nextact = "17_dbchanges7";
		$startat = $startat+$ipp;
		$contents .= "<p><input type=\"hidden\" name=\"ipspage\" value=\"$ipp\" /><input type=\"hidden\" name=\"ipstart\" value=\"$startat\" />Zakończono. Naciśnij przycisk Dalej, aby kontynuować proces.</p>";
	}
	else
	{
		$nextact = "17_redoconfig";
		$contents .= "<p>Zakończono.</p><p>Wszystkie adresy IP użytkowników zostały naprawione. Naciśnij przycisk Dalej, aby kontynuować.</p>";
	}
	$output->print_contents($contents);
...
?".">";
	...
	echo "<p>Plik z konfiguracją został pomyślnie utworzony ponownie.</p>";
