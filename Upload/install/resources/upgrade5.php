...
function upgrade5_redoconfig()
{
	...
	$output->print_header("Tworzenie nowego pliku config.php");
	...
	if($mybb->input['username'] != '' && !$mybb->input['uid'])
	{
		...
		if(!$uid)
		{
			echo "<p><span style=\"color: red; font-weight: bold;\">Wpisany login nie został odnaleziony.</span><br />Upewnij się, że wpisano poprawny login.</p>";
		}
	}
	...
	if(!$uid)
	{
		echo "<p>Wpisz login głównego administratora. Identyfikator tego użytkownika zostanie zapisany w pliku konfiguracyjnym. Zapobiegnie to zbanowaniu, zmianie danych lub usunięciu tego konta przez innego administratora.</p>";
		echo "<p>Login:</p>";
	...
	if(!$fh)
	{
		echo "<p><span style=\"color: red; font-weight: bold;\">Nie można otworzyć pliku inc/config.php</span><br />Przed rozpoczęciem procesu aktualizacji należy zmienić uprawnienia pliku inc/config.php tak, aby skrypt aktualizacji mógł zapisywać do tego pliku.</p><input type=\"hidden\" name=\"uid\" value=\"{$uid}\" />";
		$output->print_footer("5_redoconfig");
		exit;
	}
...
?".">";
	...
	fclose($fh);
	echo "<p>Nowy plik konfiguracyjny został pomyślnie zapisany.</p>";
...
{
	global $db, $output;
	$output->print_header("Przebudowywanie ostatnich postów na forum");
	echo "<p>Trwa przebudowywanie informacji o ostatnich postach na forum...</p>";
...
function upgrade5_indexes()
{
	...
	$output->print_header("Indeksowanie");
	echo "<p>Trwa sprawdzanie i tworzenie indeksów bazy danych...</p>";
