...
function upgrade3_convertavatars()
{
	...
	$output->print_header("Konwersja awatarów na pliki");
	...
	$contents .= "<p>Konwersja awatarów z $lower do $upper (Łącznie ".$cnt['avatarcount'].")</p>";
	...
	while($avatar = $db->fetch_array($query))
	{
		...
		if($ext)
		{
			...
			if(!$fp)
			{
				die("Nie można utowrzyć pliku. Sprawdź uprawnienia i odśwież stronę.");
			}
	...
	else
	{
		...
		$contents .= "<p>Zakończono</p><p>Wszystkie awatary zostały skonwertowane na pliki. W następnym kroku zostaną wykonane wymagane zapytania do bazy danych.</p>";
	}
	$output->print_contents($contents);
	$output->print_footer($nextact);
}
...
function upgrade3_dbchanges2()
{
	...
	$contents = "<p>Trwa dokonywanie wymaganych zmian w bazie danych.</p>";
	...
	$contents .= "<p>Zakończono</p>";
	$contents .= "<p>Ponowne tworzenie ustawień...";
...
function upgrade3_dbchanges3()
{
	...
	$output->print_header("Zmiana rozmiaru pól w bazie danych");

	$contents = "<p>Trwa zmiana rozmiaru pól w bazie danych.</p>";
	...
	$contents .= "<span style=\"color: red; font-weight: bold;\">OSTRZEŻENIE:</span> W następnym kroku zostaną usunięte wszystkie style i szablony zainstalowane na Twoim forum! Przed rozpoczęciem wykonaj ich kopię zapasową.</p>";
	$output->print_contents($contents);
