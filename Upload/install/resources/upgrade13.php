...
function upgrade13_dbchanges1()
{
	...
	$output->print_header("Naprawa konwersji adresów IP postów");
	...
	echo "<p>Naprawianie adresu IP z {$lower} do {$upper} (Łącznie: {$cnt['ipcount']} )</p>";
	flush();
	...
	if($remaining && $ipaddress)
	{
		...
		$startat = $startat+$ipp;
		$contents = "<p><input type=\"hidden\" name=\"ipspage\" value=\"$ipp\" /><input type=\"hidden\" name=\"ipstart\" value=\"$startat\" />Zakończono. Naciśnij przycisk Dalej, aby przejść do następnej konwersji.</p>";
	}
	else
	{
		$nextact = "13_dbchanges2";
		$contents = "<p>Zakończono</p><p>Wszystkie adresy IP zostały przekonwertowane do nowego formatu. Naciśnij przycisk Dalej, aby kontynuować.</p>";
	}
	$output->print_contents($contents);
...
function upgrade13_dbchanges2()
{
	...
	$output->print_header("Naprawa konwersji adresów IP użytkowników");
	...
	$contents .= "<p>Naprawienie adresu IP z {$lower} do {$upper} (Łącznie: {$cnt['ipcount']})</p>";
