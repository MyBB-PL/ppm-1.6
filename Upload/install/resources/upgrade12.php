...
function upgrade12_dbchanges()
{
	...
	$output->print_header("Konwersja liczb całkowitych");
	...
	else
	{
		...
		echo "<p>MyBB 1.4 to wielki krok w rozwoju projektu MyBB.</p><p>Z tego powodu, wiele danych w bazie danych musi zostać skonwertowanych do nowego formatu.</p>";
	}

	echo "<p>Trwa konwersja danych do nowego formatu, ten proces może zająć chwilę.</p>";
	...
	foreach($to_int as $table => $columns)
	{
		if($table == $current_table)
		{
			...
			if($start <= $count)
			{
				...
				echo "<p>{$table}: Konwersja {$start} do {$end} z {$count}</p>";
				flush();
			...
			{
				echo "<p>{$table}: Konwersja typu kolumn</p>";
				flush();
				$change_column = array();
				foreach($columns as $column)
				{
	...
	{
		echo "<p>Zakończono</p>";
		echo "<p>Naciśnij przycisk Dalej, aby kontynuować proces konwersji liczb całkowitych.</p>";
	...
	else
	{
		...
		echo "<p>Zakończono</p>";
		echo "<p><strong>Proces konwersji liczb całkowitych został zakończony.</strong></p>";
...
function upgrade12_dbchanges1()
{
	...
	echo "<p>Dodawanie indeksu do tabeli z prywatnymi wiadomościami... ";
	flush();
	...
	$contents = "<p>Naciśnij przycisk Dalej, aby kontynouwać proces aktualizacji.</p>";
...
function upgrade12_dbchanges_post1()
{
	...
	echo "<p>Trwa wykonywanie wymaganych zapytań do bazy danych...</p>";	
	echo "<p>Dodawanie indeksu do tabeli z postami... ";
	flush();
...
function upgrade12_dbchanges_post2()
{
	...
	{
		echo "<p>Usuwanie kolumny longipaddress z tabeli z postami... ";
		flush();
	...
	echo "<p>Dodawanie kolumny longipaddress do tabeli z postami... ";
	flush();
...
function upgrade12_dbchanges_user()
{
	...
	echo "<p>Trwa wykonywanie wymaganych zapytań do bazy danych...</p>";	
	echo "<p>Dodawanie indeksu do tabeli z użytkownikami... ";
	flush();
...
function upgrade12_dbchanges3()
{
	...
	$output->print_header("Konwertowanie banów");

	echo "<p>Trwa konwersja istniejących zbanowanych adresów IP, adresów email i loginów...</p>";
	flush();
...
function upgrade12_redoconfig()
{
	...
	$output->print_header("Ponowne tworzenie pliku config.php");
	...
	if(!is_array($config['database']))
	{
		...
		if(!$fh)
		{
			echo "<p><span style=\"color: red; font-weight: bold;\">Nie można otworzyć pliku inc/config.php</span><br />Sprawdź uprawnienia dla tego pliku przed kontynuowaniem (CHMOD 766 lub 777).</p>";
			$output->print_footer("12_redoconfig");
			exit;
		}
...
?".">";
	...
	echo "<p>Plik konfiguracyjny został pomyślnie zapisany.</p>";
	echo "<p>Naciśnij przycisk Dalej, aby kontynouwać proces aktualizacji.</p>";
...
function upgrade12_dbchanges6()
{
	...
	$output->print_header("Konwersja adresów IP postów");
	...
	echo "<p>Konwersja adresu IP z {$lower} na {$upper} (Łącznie: {$cnt['ipcount']})</p>";
	flush();
	...
	else
	{
		...
		$contents = "<p>Zakończono</p><p>Wszystkie adresy IP postów zostały skonwertowane do nowego formatu. Naciśnij przycisk Dalej, aby kontynuować.</p>";
	}
	$output->print_contents($contents);
...
function upgrade12_dbchanges8()
{
	...
	$output->print_header("Konwersja wydarzeń");
	...
	$contents .= "<p>Konwersja wydarzeń z {$lower} do {$upper} (Łącznie: {$cnt['eventcount']})</p>";
	...
	if($remaining && $date)
	{
		$nextact = "12_dbchanges8";
		$startat = $startat+$epp;
		$contents .= "<p><input type=\"hidden\" name=\"eventspage\" value=\"$epp\" /><input type=\"hidden\" name=\"eventstart\" value=\"$startat\" />Zakończono. Naciśnij przycisk Dalej, aby przejść do konwersji innego typu wydarzeń.</p>";
	}
	else
	{
		...
		$contents .= "<p>Zakończono</p><p>Wszystkie wydarzenia zostały skonwertowane. Naciśnij przycisk Dalej, aby kontynuować.</p>";
	}
	$output->print_contents($contents);
...
function upgrade12_redothemes()
{
	...
	$output->print_header("Konwersja stylów");
	...
	if($not_writable)
	{
		echo "<p><span style=\"color: red; font-weight: bold;\">Nie można zapisać do katalogu /cache.</span><br />Przed kontynuowaniem procesu aktualizacji, upewnij się, że katalog istnieje i ma nadane odpowiednie uprawnienia (CHMOD 777).</p>";
	...
	if($not_writable)
	{
		echo "<p><span style=\"color: red; font-weight: bold;\">Nie można zapisać do katalogu /cache/themes.</span><br />Przed kontynuowaniem procesu aktualizacji upewnij się, że katalog istnieje i ma nadane odpowiednie uprawnienia (CHMOD 777)</p>";
		$output->print_footer("12_redothemes");
		exit;
	}
	...
	{
		$output->print_error("Upewnij się, że katalog administracyjny (/admin) został poprawnie załadowany na serwer.");
	}

	// Import master theme
	if(import_theme_xml($contents, array("tid" => 1, "no_templates" => 1, "version_compat" => 1)) === -1)
	{
		$output->print_error("Upewnij się, że plik /install/resources/mybb_theme.xml został poprawnie załadowany na serwer.");
	}
	...
	echo "<p>Style zostały pomyślnie skonwertowane do nowego systemu.</p>";
	echo "<p>Naciśnij przycisk Dalej, aby kontynouwać proces aktualizacji.</p>";
